...
def save2vesta(
    ...
    with open(prefix + "_r.vasp", "w") as out:
        out.write(head)
        out.write("\n".join(["".join([fmt % xx for xx in row]) for row in psi_h.real]))
        out.write("\n" + "".join([fmt % xx for xx in psi_r.real]))
    if not (lgam or lreal):
        with open(prefix + "_i.vasp", "w") as out:
            out.write(head)
            out.write(
                "\n".join(["".join([fmt % xx for xx in row]) for row in psi_h.imag])
            )
...
class vaspwfc(object):
        ...
        if self._lsoc:
            ...
            if self._lgam:
                ...
                if self._gam_half == "z":
                    ...
                    tmp = (
                        np.fft.irfftn(phi_k, s=(ngrid[2], ngrid[1], ngrid[0])) * normFac
                    )
    ...
    def inverse_participation_ratio(self, band_ind_list=[], norm=True):
        """
        ...
        for ispin in range(self._nspin):
            for ikpt in range(self._nkpts):
                for iband in band_ind_list:
                    ...
                    self.ipr[ispin, ikpt, iband - 1, 1] = self._bands[
                        ispin, ikpt, iband - 1
                    ]
                    self.ipr[ispin, ikpt, iband - 1, 2] = (
            ...
            for ikpt in range(self._nkpts):
                ...
                for iband in range(self._nbands):
                    ...
                    phi_q = self.readBandCoeff(
                        ispin=ispin + 1, ikpt=ikpt + 1, iband=iband + 1, norm=False
                    )
